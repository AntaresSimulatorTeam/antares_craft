...
"""
    ...
    def test_creation_matrices_not_allowed_88(self, tmp_path: Path, local_study_w_areas) -> None:
        ...
        with pytest.raises(
                ValueError, match="The matrix STStorageMatrixName.COST_VARIATION_INJECTION is not available for study version 8.8"
        ):
            storage.get_cost_variation_injection()
        with pytest.raises(
                ValueError, match="The matrix STStorageMatrixName.COST_VARIATION_WITHDRAWAL is not available for study version 8.8"
        ):
